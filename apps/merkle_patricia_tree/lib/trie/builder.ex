defmodule MerklePatriciaTree.Trie.Builder do
  ...
  defp trie_put_key(
    ...
    {:ext, matching_prefix,
     build_branch([{old_tl, old_value}, {new_tl, new_value}], trie) |> Node.encode_node(trie)}
  end
  ...
  defp trie_put_key({:ext, shared_prefix, node_hash}, new_prefix, new_value, trie)
    ...
    {:ext, shared_prefix,
     Node.decode_trie(node_hash |> Trie.into(trie)) |> put_key([], new_value, trie)}
  end
  ...
  defp trie_put_key(
    ...
    if old_tl == [] do
      ...
      {:ext, matching_prefix,
       build_branch([first, {new_tl, new_value}], trie) |> Node.encode_node(trie)}
    end
  end

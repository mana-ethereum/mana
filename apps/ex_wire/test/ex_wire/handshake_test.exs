defmodule HandshakeTest do
  ...
  describe "new/1" do
    test "marks the owner as the initiator and sets remote public key" do
      her_static_public_key = ExthCrypto.Test.public_key(:key_b)

      handshake = Handshake.new(her_static_public_key)

      assert handshake.initiator == true
      assert handshake.remote_pub == her_static_public_key
    end
  ...
  describe "new_response/0" do
    test "marks the owner as the recipient" do
      handshake = Handshake.new_response()

      assert handshake.initiator == false
    end
    ...
    test "generates a nonce and an ephemeral key pair" do
      ...
      {public_key, private_key} = handshake.random_key_pair
      assert {:ok, ^public_key} = ExthCrypto.Signature.get_public_key(private_key)
      assert byte_size(handshake.resp_nonce) == handshake_nonce_length
    end
  end

  describe "generate_auth/1" do
    test "generates an encoded AuthMsgV4" do
      ...
      assert handshake.auth_msg == remove_ephemeral_public_key(auth_msg)
      assert auth_msg.initiator_public_key == my_static_public_key
      assert auth_msg.initiator_nonce == handshake.init_nonce
    end
  end

  describe "handle_auth/1 and handle_ack/2" do
    test "decode auth/ack via eip-8 and generate secrets" do
      my_static_private_key = ExthCrypto.Test.private_key(:key_a)
      her_static_public_key = ExthCrypto.Test.public_key(:key_b)
      her_static_private_key = ExthCrypto.Test.private_key(:key_b)
      ...
      {:ok, her_handshake, her_secrets} =
        Handshake.new_response()
        |> Handshake.handle_auth(my_handshake.encoded_auth_msg)

      # Same auth message, except remote has added the initiator ephemeral public key
      assert my_handshake.auth_msg == remove_ephemeral_public_key(her_handshake.auth_msg)
  ...
  describe "EIP-8 test vectors" do
  ...
  defp remove_ephemeral_public_key(auth_msg) do
    %{auth_msg | initiator_ephemeral_public_key: nil}
  end

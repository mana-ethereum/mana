defmodule ExWire.Struct.Bucket do
  ...
  def add_node(bucket = %Bucket{current_node: current_node}, node) do
    cond do
      Peer.equal?(current_node, node) -> {:current_node, nil, bucket}
      Bucket.member?(bucket, node) -> {:reinsert_node, node, reinsert_node(bucket, node)}
      Bucket.full?(bucket) -> {:full_bucket, head(bucket), bucket}
      true -> {:insert_node, node, insert_node(bucket, node)}
    end
  end
  ...
  ## Examples
      ...
      true
  """
  def reinsert_node(bucket = %Bucket{}, node) do
    bucket
    |> remove_node(node)
    |> insert_node(node)
  end
  ...
  def member?(%Bucket{nodes: nodes}, node) do
    ...
    |> Enum.any?(fn(bucket_node) ->
      bucket_node.remote_id == node.remote_id
    end)
  end
